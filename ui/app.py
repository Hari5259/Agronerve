...
if "orchestrator" not in st.session_state or not hasattr(
    st.session_state.orchestrator, "process_multimodal_turn"
):
...
with st.sidebar:
    ...
    selected_lang_name = st.selectbox(
        "Choose Interface Language:", list(SUPPORTED_LANGUAGES.values()), index=0
    )
        ...
        if (
            ...
            if st.session_state.selected_lang == "en":
                st.session_state.messages[0][
                    "content"
                ] = "👋 **Welcome to AgroNerve Multimodal AI!**\n\nYou can **type any farming question** or **attach a picture of your crop leaf** (e.g. Tomato, Paddy, Cotton) below.\n\nI will diagnose the plant disease, quantify foliar damage %, provide verified ICAR treatments, and let you continue chatting with full conversation memory!"
            else:
                st.session_state.messages[0][
                    "content"
                ] = f"👋 **{language_manager.get_text('welcome_msg', st.session_state.selected_lang)}**"
    ...
    sample_queries = [
        (
            "🌿 Paddy Blast Disease",
            "There are spindle-shaped brown spots with grey center on my paddy leaves",
        ),
        (
            "🧪 Stem Borer Dosage",
            "What is the recommended dosage of Chlorantraniliprole 18.5 SC for stem borer in paddy?",
        ),
        (
            "🌦️ Spray Rain Window",
            "Should I spray pesticide if heavy rainfall is forecasted tomorrow?",
        ),
        (
            "💧 Tomato Water Schedule",
            "How many days interval should I water tomato plants during flowering stage?",
        ),
        (
            "⚡ Compound: Wilt + Rain",
            "My tomato crop is wilting, should I irrigate or wait for rain tomorrow?",
        ),
    ]
...
with tab_chat:
    ...
    with st.expander(
        ...
        photo_query = st.text_input(
            "Optional question about this leaf image:",
            placeholder="e.g., What disease is this and how do I cure it?",
        )
        if uploaded_chat_photo and st.button(
            "🚀 Analyze Leaf Image & Send to AI Chat", key="btn_send_photo"
        ):
            ...
            with st.spinner(
                ...
                mm_res = st.session_state.orchestrator.process_multimodal_turn(
                    ...
                    user_text=photo_query
                    or f"Please analyze this leaf photograph of {chat_crop_hint if chat_crop_hint != 'Auto-detect' else 'my crop'} and suggest cure.",
                    session_id=st.session_state.session_id,
                    crop_hint=(
                        chat_crop_hint if chat_crop_hint != "Auto-detect" else "auto"
                    ),
                ...
                st.session_state.messages.append(
                    {
                        "role": "user",
                        "content": photo_query
                        or "Uploaded crop leaf image for diagnosis.",
                        "meta": {"has_image": True},
                        "image": photo_bytes,
                    }
                )
        ...
        with st.chat_message(msg["role"]):
            ...
            if msg.get("meta") and msg["role"] == "assistant":
                ...
                badge_class = (
                    ...
                    if is_multi
                        ...
                        f"badge-{domain}"
                        if domain in ["disease", "pesticide", "weather", "irrigation"]
                        else "badge-general"
                    )
                )
                st.markdown(
            ...
            if msg["role"] == "assistant" and len(msg["content"]) > 10:
                ...
                st.components.v1.html(
                    voice_engine.generate_html5_audio_speech_script(
                        speech_text, lang=current_lang
                    ),
                    height=45,
                )
    ...
    user_input = st.chat_input(
        language_manager.get_text("input_placeholder", current_lang)
    )
    prompt_to_process = selected_sample or user_input

    if prompt_to_process:
        st.session_state.messages.append(
            {"role": "user", "content": prompt_to_process, "meta": None, "image": None}
        )
        ...
        with c_right:
            with st.spinner("Scanning leaf visual patterns & lesion density..."):
                ...
                if scan_res.get("status") == "success":
                    ...
                    st.progress(
                        metrics.get("chlorosis_yellow_pct", 0) / 100.0,
                        text=f"Chlorosis (Yellowing): {metrics.get('chlorosis_yellow_pct')}%",
                    )
                    st.progress(
                        metrics.get("necrotic_brown_pct", 0) / 100.0,
                        text=f"Necrotic Lesions: {metrics.get('necrotic_brown_pct')}%",
                    )
                    st.progress(
                        metrics.get("dark_lesion_pct", 0) / 100.0,
                        text=f"Dark Blight Spots: {metrics.get('dark_lesion_pct')}%",
                    )
...
with tab_sensor:
    ...
    st.write(
        "Real-time telemetry integration with on-device / Bluetooth soil probes and meteorological sensors."
    )
    ...
    s1.metric(
        "🌱 Soil Moisture (VWC)",
        f"{soil['moisture_vwc_pct']}%",
        delta="Normal Hydration" if soil["status"] == "NORMAL" else "Alert",
    )
...
with tab_kb:
    ...
    st.write(
        "Browse locally stored and embedded agricultural datasets sourced from ICAR, FAO, and CIBRC."
    )

    kb_domain = st.selectbox(
        "Select Domain Partition:",
        ["All", "Disease", "Pesticide", "Weather", "Irrigation"],
    )
    ...
    for chunk in filtered:
        with st.expander(
            f"[{chunk.get('domain', '').upper()}] {chunk.get('title', 'Document')}"
        ):
...
with tab_bench:
    ...
    st.write(
        "Evaluate two-stage intent routing accuracy and RAG recall over the 40-query test benchmark."
    )
    ...
    if st.button("🚀 Run Benchmark Evaluation"):
        with st.spinner("Running 40 test queries across 4 domains..."):
            ...
            b3.metric(
                "Retrieval Recall@5", f"{bench_res['retrieval_recall_top5'] * 100}%"
            )
