...
class SensorTelemetryManager:
    ...
    def get_telemetry(self) -> Dict[str, Any]:
        ...
        self._soil_moisture_vwc = max(10.0, min(85.0, round(self._soil_moisture_vwc + random.uniform(-0.4, 0.4), 1)))
        self._soil_temp_c = round(self._soil_temp_c + random.uniform(-0.1, 0.1), 1)
        self._ambient_temp_c = round(self._ambient_temp_c + random.uniform(-0.2, 0.2), 1)
        self._ambient_humidity_rh = max(20.0, min(98.0, round(self._ambient_humidity_rh + random.uniform(-0.5, 0.5), 1)))
        ...
        if self._ambient_temp_c > 35.0:
            spray_window_status = "Unfavorable (High temperature will cause droplet scorch)"
    ...
    def set_manual_telemetry(self, soil_moisture: float, ambient_temp: float, humidity: float, rain: bool):
