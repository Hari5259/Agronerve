...
class IntentRouter:
    ...
    def route_stage1(self, query: str) -> Tuple[DomainType, float, bool]:
        ...
        if top_score == 0:
            logger.info(
                "Stage 1 routing completed: No matching keywords, fallback to general."
            )
            return "general", 0.0, False

        # Confident if top score >= 1.5x second score (or second score is 0 and top >= 2.0)
        is_confident = (
            (top_score >= self.margin_threshold * second_score)
            if second_score > 0
            else (top_score >= 2.0)
        )
        logger.info(
            f"Stage 1 routing result: top_domain={top_domain}, score={top_score}, is_confident={is_confident}"
        )
    ...
    def route_stage2_llm(self, query: str) -> DomainType:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"Ollama Stage 2 routing failed with status code: {response.status_code}"
                )
